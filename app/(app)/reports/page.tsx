...
export default function ReportsPage() {
  ...
  const totalEarnings = entries.filter(e => e.billable).reduce((s, e) => s + ((e.duration_sec || 0) / 3600) * (e.project?.hourly_rate || 0), 0)
  ...
  entries.forEach(e => {
    ...
    if (e.billable) { teamMap[e.user_id].billableSecs += e.duration_sec || 0; teamMap[e.user_id].earnings += ((e.duration_sec || 0) / 3600) * (e.project?.hourly_rate || 0) }
  })
  ...
  function exportCSV() {
    const rows = [
      ...
      ...entries.map(e => {
        ...
        return [format(parseISO(e.start_time), 'yyyy-MM-dd'), e.description || '', e.project?.name || '', e.project?.client?.name || '', member?.full_name || member?.email || e.user_id, ((e.duration_sec || 0) / 3600).toFixed(2), e.billable ? 'Yes' : 'No', e.billable ? (((e.duration_sec || 0) / 3600) * (e.project?.hourly_rate || 0)).toFixed(2) : '0']
      })
    ]
