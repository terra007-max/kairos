            ...
            {members.map(m => (
              <div key={m.id} className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg group border border-transparent hover:border-border transition-colors">
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  ...
                  {m.role === 'project_manager' && <span className="inline-flex items-center gap-1 text-[10px] font-medium text-brand-600 dark:text-brand-400 bg-brand-500/10 px-1.5 py-0.5 rounded-full">PM</span>}
                  {m.status === 'pending' && (
